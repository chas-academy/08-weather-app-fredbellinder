        ...
        return (
            <div className={"hourly"}>
                <ul className={"list-group hourly"}>
                    <li key={1} className={"list-item hourly title"}>TriHourly:</li>
                    {renderArray.map(data => {
                        return (
                            <li key={data.time} className={"list-item hourly"}>

                                {props.unixTimestampToLocaleTimeStringInLocalTimeZone(data.time)}
                                <p>
                                    <img
                                        ...
                                        alt={data.icon}
                                    />
                                    Temp: {props.celsius ? fahrenheit(data.temperature) : celsius(data.temperature)}
                                </p>
                            </li>
                        )
                    })}
